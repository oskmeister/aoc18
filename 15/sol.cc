...
bool act(State &state, int r, int c) {
    ...
    if (!enemies_nearby.empty()) {
        ...
        cout << "--> " << r << " " << c << " hits " << best->r << " " << best->c << endl;
        best->hp -= 3;
        if (best->hp <= 0) {
            // Die!
...
bool simulate_round(State &state) {
    auto &grid = state.grid;
    auto &units = state.units;
    ...
    for (int r = 0; r < grid.size(); ++r) {
        for (int c = 0; c < grid[0].size(); ++c) {
            char ch = grid[r][c];
            if (units[r][c]) ch = (units[r][c]->type ? 'G' : 'E');
            cout << ch;
        }
        cout << endl;
    }
    ...
    for (int r = 0; r < grid.size(); ++r) {
        for (int c = 0; c < grid[0].size(); ++c) {
            if (units[r][c]) cout << units[r][c]->hp << endl;
        }
    }
...
int main() {
    ...
    for (int r = 0; r < s.grid.size(); ++r) {
        for (int c = 0; c < s.grid[0].size(); ++c) {
            if (s.units[r][c]) sumhp += s.units[r][c]->hp;
        }
    }
